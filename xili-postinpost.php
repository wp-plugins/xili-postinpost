...
class xili_postinpost {
	...
	function xili_widgets_init() {
		load_plugin_textdomain( 'xili_postinpost',false, 'xili-postinpost' );
	...
	function on_sidebox_mail_content ( $data ) {
		...
		<label for="xiliplugenable">
			<input type="checkbox" id="xiliplugenable" name="xiliplugenable" value="enable" <?php if( $this->xili_settings['xiliplug']=='enable') echo 'checked="checked"' ?> />&nbsp;<?php printf(__("Other xili plugins = %s", "xili_postinpost"), $list  ); ?>
		</label><br /><br />
		<?php } ?>
		<label for="webmestre"><?php _e('Type of webmaster:','xili_postinpost'); ?>
		<select name="webmestre" id="webmestre" style="width:100%;">
			<option value="?" ><?php _e('Define your experience as webmaster…','xili_postinpost'); ?></option>
			<option value="newbie" ><?php _e('Newbie in WP','xili_postinpost'); ?></option>
			<option value="wp-php" ><?php _e('Good knowledge in WP and few in php','xili_postinpost'); ?></option>
			<option value="wp-php-dev" ><?php _e('Good knowledge in WP, CMS and good in php','xili_postinpost'); ?></option>
			<option value="wp-plugin-theme" ><?php _e('WP theme and /or plugin developper','xili_postinpost'); ?></option>
		</select></label>
		<br /><br />
	...
	function xili_postinpost_settings () {
		...
		if ( isset($_POST['sendmail']) ) {
			...
			$contextual_arr[] = $_POST['webmestre'];
...
class xili_post_in_post_Widget extends WP_Widget {
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['userfunction'] = strip_tags($new_instance['userfunction']); // 1.0.0
...
?>
		...
		<p><label for="<?php echo $this->get_field_id('userfunction'); ?>"><?php _e('Function (must exists)','xili_postinpost'); ?></label><input id="<?php echo $this->get_field_id('userfunction'); ?>" name="<?php echo $this->get_field_name('userfunction'); ?>" type="text" value="<?php echo $userfunction; ?>" size="50" />
		</p>
