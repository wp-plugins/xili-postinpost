...
class xili_postinpost {
	...
	function localize_admin_js( $case_news, $news_id ) {
			...
			switch ( $case_news ) {
				...
				$pointer_text .= '<p>' . esc_js( sprintf(__( 'This version %s improves query when done in front page with xili-language active.', 'xili_postinpost'), XILI_PIP_VERSION) ). '.</p>';

				$pointer_text .= '<p>' . esc_js( sprintf(__( 'The previous versions of %s add previewing feature for theme/customize/widget (WP 3.9+) and a new filter for nopost results (developers), and add the new params “more” for content part in widget [shortcode]', 'xili_postinpost'), XILI_PIP_VERSION) ). '.</p>';
